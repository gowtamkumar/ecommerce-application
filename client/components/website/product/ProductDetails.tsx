        ...
        <div className="relative bg-white rounded-3xl p-6 lg:p-8 shadow-xl shadow-indigo-500/5 border border-white ring-1 ring-gray-100 overflow-hidden">
          ...
          <div className="relative flex items-center justify-between mb-6 z-10">
            ...
            {checkStock > 0 ? (
                <div className="flex items-center gap-2">
                   ...
                   <span className="text-emerald-700 font-bold text-sm tracking-wide">In Stock</span>
                </div>
              ) : (
          ...
          <div className="relative flex items-center gap-4 mb-8 pb-8 border-b border-gray-100 z-10">
             <div className="flex items-center gap-1.5 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">
                ...
                />
                <span className="text-gray-900 font-bold text-sm pt-0.5">{averageRating}</span>
             </div>
             <span className="text-gray-400 text-sm font-medium">
               Based on <strong className="text-gray-900">{+reviewsCount || 0} reviews</strong>
             </span>
          </div>
          ...
          {variant && (
            <div className="relative space-y-6 mb-10 z-10">
              <div className="space-y-4">
                <span className="text-sm font-extrabold text-gray-900 uppercase tracking-widest">Select Variant</span>
                <div className="flex flex-wrap gap-3">
                  {(productVariants || []).map((item: any, idx: number) => {
                    ...
                    return (
                      ...
                      >
                         {item?.color?.value ? (
                          <div className={`p-1 rounded-full border ${isSelected ? "border-indigo-200" : "border-gray-100"}`}>
                             <div
                              className="w-5 h-5 rounded-full shadow-sm"
                              style={{ backgroundColor: item.color.value }}
                            />
                          </div>
                        ) : <div className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center font-bold text-xs text-gray-500">?</div>}
                        
                        <div className="flex flex-col items-start">
                          <span className={`font-bold text-sm ${isSelected ? "text-indigo-900" : "text-gray-700"}`}>{item?.size?.name}</span>
             ...
             <div className="flex items-center justify-between gap-4 pt-2">
                ...
                >
                   <div className="w-10 h-10 rounded-full bg-gray-50 flex items-center justify-center group-hover:bg-rose-50 transition-colors">
                      <CiHeart size={22} className="group-hover:scale-110 transition-transform" />
                   </div>
